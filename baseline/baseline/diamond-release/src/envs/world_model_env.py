...
class WorldModelEnv:
    ...
    @torch.no_grad()
    def reset(self, frames, actions, offset, **kwargs) -> ResetOutput:
        # obs, obs_full_res, act, next_act, (hx, cx) = self.generator_init.send(self.num_envs)
        obs, obs_full_res, act, next_act, (hx, cx) = self.mc_init(frames, actions, offset)
        self.obs_buffer = obs
        self.act_buffer = act
    ...
    def mc_init(self, frames, actions, offset):
        # required obs in [-1,1]
        # obs shape = [1,T,3,H,W] ?
        # T = seq_length
        # frames [180,320]; obs [36,64]; 
        frames = frames[offset - self.seq_length:offset]
        frames = torch.from_numpy(frames).to(self.device)
        actions = actions[offset - self.seq_length:offset]
        obs = T.resize(frames, (36,64), interpolation=T.InterpolationMode.BICUBIC)
        obs_full_res = frames
        obs = obs.unsqueeze(0).to(self.device)
        obs_full_res = obs_full_res.unsqueeze(0).to(self.device)
        act = actions.clone().unsqueeze(0).to(self.device)

        next_act = None
        hx = None
        cx = None
        return obs, obs_full_res, act, next_act, (hx, cx)